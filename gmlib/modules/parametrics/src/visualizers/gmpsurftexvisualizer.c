...
#include "gmpsurftexvisualizer.h"
...
namespace GMlib {
  ...
  PSurfTexVisualizer<T,n>::PSurfTexVisualizer()
  ...
  template <typename T, int n>
  PSurfTexVisualizer<T,n>::PSurfTexVisualizer(const PSurfTexVisualizer<T,n>& copy)
  ...
  template <typename T, int n>
  void PSurfTexVisualizer<T,n>::setTexture(const GL::Texture& tex) {
  ...
  void PSurfTexVisualizer<T,n>::render( const SceneObject* obj, const DefaultRenderer* renderer ) const {
    ...
    _prog.bind(); {
      ...
      // Normal map
      _prog.uniform( "u_nmap", _nmap, static_cast<GLenum>(GL_TEXTURE0), 0 );

      // Texture
      _prog.uniform( "u_tex", _tex, static_cast<GLenum>(GL_TEXTURE1), 1 );
  ...
  template <typename T, int n>
  inline
  void PSurfTexVisualizer<T,n>::draw() const {
  ...
  template <typename T, int n>
  inline
  void PSurfTexVisualizer<T,n>::replot(
  ...
  template<typename T,int n>
  void PSurfTexVisualizer<T,n>::initShaderProgram() {

    const std::string name         = "psurf_tex_visu";
    const std::string prog_name    = name + "_prog";
